...
export const aiService = {
  async explainCode(code: string, language: string): Promise<string> {
    ...
    return (
      response.choices[0]?.message?.content || "Failed to generate explanation."
    );
  },
      ...
      messages: [
        {
          role: "system",
          content:
            "Detect the programming language of the following code. Return ONLY the language name in lowercase (e.g., javascript, python, html). Do not include any other text.",
        },
        {
          role: "user",
